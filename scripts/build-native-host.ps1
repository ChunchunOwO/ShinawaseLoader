...
param(
  ...
  # (echo-steam 26.8.28 / Chromium 150). Never default to 42.3.1.
...
# ECHO.exe FileVersion is the app stamp (26.8.28), not Electron.
...
if (-not $PSBoundParameters.ContainsKey('ElectronVersion') -or -not $ElectronVersion) {
  ...
  Write-Warning "Ignoring non-Electron version '$ElectronVersion' (ECHO.exe FileVersion 26.8.28 is the app stamp). Resolving ABI from $EchoRoot or $FallbackElectron."
  $ElectronVersion = Resolve-EchoElectronVersion $EchoRoot
}
...
try {
  ...
  Write-Host "Building echo-native-host for $Runtime $ElectronVersion (echo-steam 26.8.28 ABI)"
  npx @gypArgs
