...
The script targets Electron **43.3.0** headers by default (echo-steam **26.8.28**, Chromium 150, Node `>=22.23.2 <23`). Resolution order:
...
Do not pass app FileVersion **26.8.28** to node-gyp — that is not an Electron ABI. Use `-Runtime node` only when testing the addon under the bundled Node runtime; ECHO in-process loading needs the Electron ABI.
