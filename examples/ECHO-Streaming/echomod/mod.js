...
const mapMainPlaylistTracks = (listed, provider, fallbackName) => {
  const tracks = (Array.isArray(listed?.tracks) ? listed.tracks : []).map((item) => {
    const providerTrackId = String(item?.providerTrackId || '').trim();
    if (!providerTrackId) return null;
...
const loadQqPlaylistTracksViaMain = async (playlist, fallbackName) => {
  ...
  return mapMainPlaylistTracks(listed, 'qqmusic', fallbackName);
};
...
const openImportedPlaylist = async (imported) => {
  ...
  // overlay on a native nav-item click. ECHO Next dropped data-workshop-icon
  // on those buttons, so also match the visible 播放列表 label.
  const nav = document.querySelector('button.nav-item[data-workshop-icon="nav-playlists"]')
    || [...document.querySelectorAll('button.nav-item:not([data-echo-external-sidebar]):not([data-echo-external-loader]):not([data-echo-external-mods])')]
      .find((button) => /播放列表|Playlists/iu.test(button.textContent || ''));
