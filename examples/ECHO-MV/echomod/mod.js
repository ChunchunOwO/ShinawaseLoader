...
const renderOffset = () => {
  ...
  startCard.append(
    ...
    btn('mv-offset-replay-button', {
      ...
      onclick: () => {
        notifyMvChanged(state.trackId);
        const seconds = Math.max(0, (Number(state.selectedVideo?.offsetMs) || 0) / 1000);
        void echoApi().audio?.seek?.(seconds)?.catch?.(() => undefined);
        void replayCurrent();
      },
...
const startTimers = () => {
  const tick = () => {
    ...
    if (panelActive() && shouldFollowMusic(state.settings, state.selectedVideo)) {
      if (state.isAudioPlaying) syncVideos();
      else {
