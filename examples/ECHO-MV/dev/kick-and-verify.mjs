// Dev-only: kick playback, wait for the MV to remount, verify CSS + visuals.
...
// kick playback
console.log('kick:', JSON.stringify(await evalJson(`(async () => {
  try {
    const status = await window.echo?.audio?.getStatus?.().catch(() => null);
...
// wait for video to remount and play (up to 30s)
let last = '';
for (let s = 0; s < 30; s += 1) {
  ...
  if (snap.hasVideo && snap.paused === false && snap.t > 3) break;
}

await new Promise((r) => setTimeout(r, 1500));
const shot = await call('Page.captureScreenshot', { format: 'png' });
writeFileSync('examples/ECHO-MV/dev/mv-live-final.png', Buffer.from(shot.data, 'base64'));
console.log('screenshot: examples/ECHO-MV/dev/mv-live-final.png');
