// Dev-only: manually select the top candidate for the current track and
// verify the video element loads through the echo-mv:// proxy protocol.
const CDP_PORT = 9229;
const list = await (await fetch(`http://127.0.0.1:${CDP_PORT}/json/list`)).json();
const target = list.find((t) => t.type === 'page' && /index\.html/i.test(t.url || ''));
if (!target) { console.error('no ECHO page target'); process.exit(1); }
const ws = new WebSocket(target.webSocketDebuggerUrl);
await new Promise((res, rej) => { ws.addEventListener('open', res, { once: true }); ws.addEventListener('error', rej, { once: true }); });
const state = { id: 0, pending: new Map() };
ws.addEventListener('message', (e) => { const m = JSON.parse(e.data); if (m.id && state.pending.has(m.id)) { const p = state.pending.get(m.id); state.pending.delete(m.id); m.error ? p.rej(new Error(JSON.stringify(m.error))) : p.res(m.result); } });
const call = (method, params = {}) => new Promise((res, rej) => { const id = ++state.id; state.pending.set(id, { res, rej }); ws.send(JSON.stringify({ id, method, params })); setTimeout(() => { if (state.pending.has(id)) { state.pending.delete(id); rej(new Error('timeout')); } }, 60000); });
await call('Runtime.enable');
const evaluate = async (expression) => {
  const r = await call('Runtime.evaluate', { expression, awaitPromise: true, returnByValue: true });
  if (r.exceptionDetails) return { error: (r.exceptionDetails.exception?.description || 'eval error').slice(0, 700) };
  return { value: r.result?.value };
};
const out = (label, data) => console.log(`\n=== ${label} ===\n${typeof data === 'string' ? data : JSON.stringify(data, null, 1)}`);

const sel = await evaluate(`(async () => {
  const a = await window.echo.audio.getStatus();
  const mv = window.__echoShinawaseStreaming.mv;
  const trackId = a.currentTrackId;
  const cands = await mv.getCandidates(trackId);
  if (!Array.isArray(cands) || !cands.length) return { error: 'no candidates for ' + trackId };
  const pick = cands.find((c) => c.playableInApp) || cands[0];
  const video = await mv.selectVideo(trackId, pick.id);
  return {
    trackId,
    picked: { t: (pick.title || '').slice(0, 46), s: pick.score },
    selected: video ? { title: (video.title || '').slice(0, 46), playable: video.playableInApp, mediaUrl: (video.mediaUrl || '').slice(0, 70), quality: video.qualityLabel, origin: video.selectionOrigin } : null,
  };
})()`);
out('selectVideo', sel.value ?? sel.error);

let panel = null;
for (let i = 0; i < 20; i += 1) {
  await new Promise((r) => setTimeout(r, 1000));
  const p = await evaluate(`(() => {
    const mod = document.querySelector('[data-echo-mv-mod]');
    const video = mod ? mod.querySelector('video') : null;
    const bg = document.querySelector('.lyrics-mv-background');
    return {
      modMounted: !!mod,
      children: mod ? [...mod.children].map((c) => String(c.className).split(' ')[0]).slice(0, 8) : null,
      hasVideo: !!(video || (bg && bg.querySelector('video'))),
      videoSrc: (video || bg?.querySelector('video')) ? String((video || bg.querySelector('video')).currentSrc || '').slice(0, 90) : null,
      videoReady: (video || bg?.querySelector('video'))?.readyState ?? null,
      videoW: (video || bg?.querySelector('video'))?.videoWidth ?? null,
      playing: (video || bg?.querySelector('video')) ? !(video || bg.querySelector('video')).paused : null,
      immersiveBg: !!bg,
    };
  })()`);
  panel = p.value ?? { error: p.error };
  if (panel.hasVideo && panel.videoReady >= 2 && panel.videoW > 0) break;
}
out('video state', panel);

const shot = await call('Page.captureScreenshot', { format: 'png' });
const { writeFileSync } = await import('node:fs');
const file = new URL(`./select-test-${Date.now()}.png`, import.meta.url);
writeFileSync(file, Buffer.from(shot.data, 'base64'));
out('screenshot', decodeURIComponent(file.pathname));
try { ws.close(); } catch {}
process.exit(0);
