// Dev-only: seek the MV background video and verify frames actually composite.
const CDP_PORT = 9229;
...
const r = await call('Runtime.evaluate', { expression: `(async () => {
  ...
  const out = { before: v.currentTime };
  let frames = 0;
  if (typeof v.requestVideoFrameCallback === 'function') {
    const onFrame = () => { frames += 1; v.requestVideoFrameCallback(onFrame); };
    v.requestVideoFrameCallback(onFrame);
  }
  v.currentTime = 95;
  try { await v.play(); } catch (e) { out.playErr = String(e && e.message || e); }
  await new Promise((r2) => setTimeout(r2, 2500));
  out.after = v.currentTime;
  out.paused = v.paused;
  out.frames = frames;
  v.pause();
  return out;
